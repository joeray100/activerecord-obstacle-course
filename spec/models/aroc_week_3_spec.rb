...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      # users = User.joins(:order_items).select(:name).distinct.where('order_items.item_id = ?', 8).order(:name)

      users = User.names_of_users_ordered_one_item(@item_8.id)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------------------------------------------------
      items_for_user_3_third_order =
      Order.all.where(user_id: @user_3.id)[2].items.pluck(:name)

       # Order.all.where(user_id: 3, id: @order_9.id).items.pluck(:name)
    # ------------------ Using ActiveRecord ----------------------
